...
batch1 = mnist.test.next_batch(50) #batch大小设置为50

print("test accuracy %g" %accuracy.eval(session = sess,
      feed_dict = {x:batch1[0], y_:batch1[1],
                   keep_prob:1.0})) #神经元输出保持不变的概率 keep_prob 为 1，即不变，一直保持输'''
#x:mnist.test.images, y_:mnist.test.labels,
train_writer.close()
#写到指定的磁盘路径中
end = time.clock() #计算程序结束时间
#print("running time is %d s") % (end)
print("time running %g" %(end-start))